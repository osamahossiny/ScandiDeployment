...
data.data.categories.forEach(element => {
    insertions["category"]+=`INSERT INTO category (name) VALUES('${element.name}');\n`;
});
data.data.products.forEach(element => {
    let properties = `'${element.id}', '${element.name}', ${element.inStock.toString().toUpperCase()}, '${element.description}', '${element.category}', '${element.brand}'`
    insertions["product"]+=`INSERT INTO product (id, name, inStock, description, category, brand) VALUES(${properties});\n`;
    
    element.gallery.forEach(link => {
        properties = `'${link}', '${element.id}'`
        insertions["product_gallary"]+=`INSERT INTO product_gallary (link, product_id) VALUES(${properties});\n`;
    });
    
    element.attributes.forEach(att => {
        properties = `'${att.id}', '${att.name}', '${att.type}'`
        if (!occ.attribute.includes(att.id)) {
            insertions["attribute"]+=`INSERT INTO attribute (id, name, type) VALUES(${properties});\n`;
            occ.attribute.push(att.id);
        }
        
        att.items.forEach(item => {
            properties = `'${element.id}', '${att.id}', '${item.displayValue}', '${item.value}', '${item.id}'`
            insertions["product_attribute"]+=`INSERT INTO product_attribute (product_id, attribute_id, display_value, value, id) VALUES(${properties});\n`;
        });
    });
    element.prices.forEach(price => {
        properties = `'${price.currency.label}', '${price.currency.symbol}'`
        if (!occ.currency.includes(price.currency.label)) {
            insertions["currency"]+=`INSERT INTO currency (label, symbol) VALUES(${properties});\n`;
            occ.currency.push(price.currency.label);
        }
        properties = `'${element.id}', ${price.amount}, '${price.currency.label}'`
        insertions["price"]+=`INSERT INTO price (product_id, amount, currency) VALUES(${properties});\n`;
    });
});
let output = { 
    // used an object so can cascade in vs code :)
    output:
            'use task;\n' +
            '\n-- category Insertions' + '\n\n' +
            insertions["category"] + '\n' +
            ...
            insertions["product_gallary"] + '\n'+
            '-- attribute Insertions' + '\n\n' +
            insertions["attribute"] + '\n' +
            '-- product_attribute Insertions' + '\n\n' +
            insertions["product_attribute"] + '\n' +
            '-- currency Insertions' + '\n\n' +
            insertions["currency"] + '\n' +
            '-- price Insertions' + '\n\n' +
            insertions["price"]
